...
function EvidenceDecisionButtons({ value, disabled, busy = false, compact = false, onChange }: {
  ...
  const disabledCursor = disabled ? (busy ? 'progress' : 'not-allowed') : undefined;
  return <div className={`evaluation-decision-buttons${compact ? ' compact' : ''}`}>
    <button className={value === 'approve' ? 'active approve' : ''} type="button" disabled={disabled} aria-busy={busy || undefined} style={{ cursor: disabledCursor }} onClick={() => onChange('approve')} title="通过"><LineIcon name="check" />{!compact && <span>通过</span>}</button>
    <button className={value === 'reject' ? 'active reject' : ''} type="button" disabled={disabled} aria-busy={busy || undefined} style={{ cursor: disabledCursor }} onClick={() => onChange('reject')} title="不通过"><LineIcon name="x" />{!compact && <span>不通过</span>}</button>
    <button className={value === 'needs_attention' ? 'active attention' : ''} type="button" disabled={disabled} aria-busy={busy || undefined} style={{ cursor: disabledCursor }} onClick={() => onChange('needs_attention')} title="需关注"><LineIcon name="pin" />{!compact && <span>需关注</span>}</button>
  </div>;
}
                          ...
                          {report.reviewMatrix.groups.map((group) => {
                            ...
                            return <article key={`${group.candidateId}:${group.caseId}`} className={`evaluation-review-cell ${evidenceDecisionClass(group.decision)}`}>
                              ...
                              <div className="evaluation-review-cell-actions">
                                <EvidenceDecisionButtons value={group.humanReviewed ? group.decision : null} compact busy={saveState.status === 'saving'} disabled={report.run.status !== 'awaiting_review' || saveState.status === 'saving'} onChange={decision => { void saveCaseReview(group, decision); }} />
        ...
        <div className="evaluation-evidence-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) { setSelectedGroupKey(''); setSelectedAttempt(null); } }}>
          <aside className="evaluation-evidence" aria-label="评测证据">
            ...
            <section><div className="evaluation-subhead"><h3>重复尝试</h3><span>展开证据或直接改判</span></div><div className="evaluation-evidence-attempts">{selectedReviewGroup.attempts.map((attempt) => {
              const key = attemptReviewKey(attempt);
              const saveState = evidenceSaveState[key] || { status: 'idle' as const };
              return <article key={attempt.id} className={`${selectedAttempt?.id === attempt.id ? 'selected ' : ''}${evidenceDecisionClass(attempt.effectiveDecision)}`}><button className="evaluation-evidence-attempt-main" type="button" onClick={() => { setSelectedAttempt(attempt); setEvidenceComments(comments => ({ ...comments, [key]: comments[key] ?? attempt.reviews[0]?.comment ?? '' })); }}><span>#{attempt.repetition}</span><strong>{attempt.finalScore == null ? '-' : attempt.finalScore.toFixed(1)}</strong><em>{formatDuration(attempt.durationMs)}</em></button><EvidenceDecisionButtons value={attempt.reviewDecision} compact busy={saveState.status === 'saving'} disabled={report?.run.status !== 'awaiting_review' || saveState.status === 'saving'} onChange={decision => { void saveAttemptReview(attempt, decision); }} /><span className={`evaluation-save-state ${saveState.status}`}>{saveState.status === 'saving' ? '保存中' : saveState.status === 'saved' ? '已保存' : saveState.status === 'error' ? '重试' : ''}</span></article>;
            })}</div></section>
            {selectedAttempt && <>
